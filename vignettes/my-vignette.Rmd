...
The fitting is performed using the function `runModel`. The functions fits the model described in Diana et al. (2020). The following model configurations can be chosen:
...
* `usingSpatial`: when set to `TRUE`, this commands fits the model using spatial autocorrelated random effects for the occupancy probability (in addition to individual spatial random effects).
